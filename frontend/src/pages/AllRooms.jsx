...
const AllRooms = () => {
    ...
    const handleFilterChange = (checked, value, type) => {
        setSelectedFilters( (pf) => {
            const updatedFilters = {...pf};
    ...
    <div className="flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32">
        <div className="">
            <div className="flex flex-col items-start text-left">
                ...
                <p className="text-sm md:text-base text-gray-500/90 mt-2 max-w-174">Take advantage of blah blah</p>
            </div>

            {rooms_store.map((room) => (
                <div key={room._id} className="flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0">
                    <img onClick={() => {navigate(`/rooms/${room.id}`); scrollTo(0, 0)}} src={room.hotel_image_url[0]} alt="" title="View room details" className="max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer" />
